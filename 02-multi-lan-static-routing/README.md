...
I began by creating two additional LANs, using /16 and /8 IPv4 addressing schemes and naming the devices within each LAN appropriately. To challenge myself further, I decided to split the original /24 network into /25 subnets to simulate an additional department being added within the same building as the original LAN.

- **Insert pic of all created LANs**
...
- **Insert pic of SAT01-R01 connected between /25 networks**

### Step 3

Originally, I had planned to use just three routers: one separating the simulated departments within the SAT01 site, and one each at SAT02 and SAT03. However, I was curious about how default routes worked. I understood theoretically that a default route acts as the least-specific route and can still be selected when no more-specific route matches a packet’s destination, but I wanted to see that behavior in action within my own network.

I decided to place an additional router, TRANSIT-R01, between SAT01, SAT02, and SAT03. On TRANSIT-R01, I planned to configure specific static routes for two paths while using a default route to forward traffic through the third path. Theoretically, this should still allow communication between all networks as long as the routing information and return paths were configured correctly.

- **Insert all router infrastructure pic**
...
- **PIC OF DEFAULT GATEWAY AND ROUTER IP**

### Step 5

The final step was to bring everything together—now it was time to actually configure my frankenetwork. I had a logical framework for what needed to be done, but the closer I got to implementing everything, the more I realized I needed a checklist. There were a lot of moving parts and a lot of individual configurations that had to be correct for the network to function as intended.

I ended up spending far more time planning and organizing what needed to be configured than I did actually entering commands on the devices themselves.
...
- **Insert `show ip route` screenshots of all routers**
